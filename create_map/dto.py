...
def main():
    """
    Example usage of command line parsing.
    """
    try:
        # Parse command line arguments
        args_dict = parse_command_line_args() if len(sys.argv) > 1 else parse_command_line_args(
            [
                '--id', 'cd259c503f04558b8fa2e3f95de686b9',
                '--request_type', 'create_map',
                '--map_w', '447136',
                '--map_s', '70883',
                '--map_e', '453851',
                '--map_n', '75370',
                '--epsg', 'EPSG:3794',
                '--raster_type', 'dtk50',
                '--raster_source', 'C:\\Users\\Lenart\\Desktop\\DTK50\\res',
                '--map_size_w_m', '0',
                '--map_size_h_m', '0',
                '--target_scale', '25000',
                '--edge_wgs84', 'true',
                ...
                '--dodatno', 'Izdelal RJ� za potrebe orientacije. Karta ni bila reambulirana.',
                '--slikal', '',
                '--slikad', '',
                '--control_points', '{"cp_size":0.003,"cps":[]}',
                '--output_folder', 'C:\\Users\\Lenart\\Desktop\\topodtk\\.create_map_cache'
            ]
        )
        ...
        request = create_request_from_args(args_dict)
        
        # Print validated request
        print(f"Created request: {request.dict()}")
        print(f"Request type: {request.__class__.__name__}")
        
        # Now you can use the request object for your map creation logic
